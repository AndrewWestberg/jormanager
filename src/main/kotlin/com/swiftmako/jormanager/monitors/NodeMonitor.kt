...
//        }
        ...
        private suspend fun monitorNodeLocal(node: Node) {
            val ekgService =
                retrofit
                    .newBuilder()
                    .baseUrl("http://127.0.0.1:${node.ekgPort}")
                    .build()
        ...
        private suspend fun monitorNodeRemote(
            host: Host,
            node: Node
        ) {
        ...
        private suspend fun monitorNode(
            ...
            val epochLength =
                fileRepository.findByIdOrNull(node.genesisShelleyFileId)?.let { genesisShelleyFile ->
            ...
            while (true) {
                ...
                try {
                    ...
                    if (ekgMetrics.cardano.node.metrics.blockNum.int.valX > 0L) {
                        ...
                        val nodeStats = ekgMetrics.toNodeStats(now, node, epochLength)
        ...
        private fun EkgMetrics2.toNodeStats(
            timestamp: Long,
            node: Node,
            epochLength: Long
        ): NodeStats =
            NodeStats(
                ...
                color = node.color,
                peers =
                    this.cardano.node.metrics.connectionManager.outgoingConns.valX
                        .toInt(),
                incomingPeers =
                    this.cardano.node.metrics.connectionManager.incomingConns.valX
                        .toInt(),
                blockHeight = this.cardano.node.metrics.blockNum.int.valX,
                remainingKESPeriods =
                    this.cardano.node.metrics.remainingKESPeriods.int.valX
                        .toInt(),
